...
async function init() {
  ...
  processedData.forEach((d) => {
      ...
      // --- NEW CODE: Scale down by 10 ---
      // Only assign a position to every 10th satellite
      if (count % 10 === 0) {
        d.showInBarChart = true; // Flag this to draw it later

        const visualCount = count / 10; // Calculate its grid position based on the scaled amount
        ...
        d.showInBarChart = false; // Flag the other 9 to be ignored
      }
      // ----------------------------------

      // Keep the actual count going up by 1 so your total stats remain accurate
      typeStacks.set(g, count + 1);
  });
